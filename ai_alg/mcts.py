...
class Node:
    ...
    def add_children(self) -> None:
        ...
        if (len(self.children) != 0) or (game.available_moves(
        ...
        for col in game.available_moves(self.board):  # itera sobre todas as colunas possíveis a serem jogadas
            ...
            self.children.append((Node(board=copy_board, last_player=self.current_player, parent=self),
                                  col))  # adiciona cada tabuleiro gerado a uma lista, junto com a identificação da coluna que o gerou
    ...
    def search(self, max_time: int) -> int:
        ...
        while time.time() - start_time < max_time:
            ...
            if selected_node.visits == 0:  # se o nó ainda não tiver sido visitado numa simulação:
                ...
                selected_children = self.expand(
                    selected_node)  # se o nó já tiver sido simulado, expande e escolhe 4 filhos para simular
